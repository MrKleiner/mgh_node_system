...
function event_rehandlers() {
  ...
  checkbox_activator(
    "export_encrypted_checkbox",
    $("#config_storage").attr("export_encrypted")
  );
}
...
function creds_super_copier() {
  $(".super_node_creds_copy_this_data").click(function () {
    // node_creds_email
    var current_container = this.closest(".super_node_creds_row");
    var what_to_copy = $(current_container)
      .find(".super_node_creds_input")
      .val();
  ...
  $(".super_node_super_smart_copier").click(function () {
    ...
    if (event.ctrlKey) {
      var current_login = $(this)
        .closest(".super_node_creds_block")
        .find(".node_creds_email")
        .val();
      var current_pswd = $(this)
        .closest(".super_node_creds_block")
        .find(".node_creds_pswd")
        .val();
  ...
  $(".super_node_ctrl_move_r_btn").click(function () {
    if (event.shiftKey) {
      if (super_node_move_mode) {
        ...
        window.preset_reader_fuck_js_current_element = $(this).closest(
          ".super_node"
        );
        super_node_spawner();
        clearSelection();
  ...
  $(".super_node_content_text").click(function () {
    ...
    if (event.altKey) {
      if (hitman_mode) {
        ...
        $(this)
          .closest(".super_node_content_row")
          .addClass("global_trash_to_flush");
      ...
      if ($(this).attr("mgh_link_btn_valid") > 0) {
        if ($(this).attr("mgh_link_btn_valid").length < 5) {
          ...
          $("#global_super_link_editor")
            .find(".content_link_editor_input")
            .val(grab_link_for_input);
...
function super_row_content_link_editor_deactivator() {
  $(".content_link_editor .content_link_editor_btns").click(function () {
    ...
    var grab_val = $(current_container)
      .find(".content_link_editor_input")
      .val();
    ...
    if (grab_val.includes("https://mega")) {
      $(current_link_edited_element).css(
        "background-position",
        "-63px -71.5px"
      );
...
$(document).ready(function () {
  document.addEventListener("keydown", function (zEvent) {
    if (zEvent.altKey && zEvent.keyCode == 81) {
      // case sensitive
...
document.addEventListener("keydown", function (zEvent) {
  if (zEvent.shiftKey && zEvent.keyCode == 69) {
    // case sensitive
    // DO YOUR STUFF HERE
    window.hitman_mode ^= true;
    $(".super_util_del_rows").toggleClass("btn_blue_active_bg");
  ...
  client.onreadystatechange = function () {
    ...
    if (client.readyState == 4) {
      ...
      if (client.responseText.includes("shit_is_not_encrypted")) {
        ...
        var decrypted_shit = CryptoJS.AES.decrypt(
          client.responseText,
          cur_session_password
        );
        document.querySelector(
          ".super_canvas"
        ).innerHTML = decrypted_shit.toString(CryptoJS.enc.Utf8);
      }
...
function storage_left_editor_activator() {
  $(".super_node_storage_btn").click(function () {
    ...
    let current_node_storage_status = $(current_node).find(
      ".node_space_left_super_num"
    );
...
$(document).ready(function () {
  ...
  $(".standard_checkbox_1").click(function () {
    ...
    // $(".super_util_del_nodes").toggleClass("btn_blue_active_bg");
    var attr_check = $(this)
      .find(".disaster_checkbox_checkmark")
      .attr("mgh_checkbox_checked");
    attr_check ^= true;
    // v = 1 - v;
    $(this).find(".disaster_checkbox_checkmark").toggleClass("class_hidden");
    $(this)
      .find(".disaster_checkbox")
      .toggleClass("disaster_checkbox_checked_bg");
    $(this)
      .find(".disaster_checkbox_checkmark")
      .attr("mgh_checkbox_checked", attr_check);
  });

  $(".export_encrypted_checkbox_row").click(function () {
    $("#config_storage").attr(
      "export_encrypted",
      1 - $("#config_storage").attr("export_encrypted")
    );
  });
});
// }
...
function checkbox_activator(cbid, cstate) {
  ...
  if (cstate > 0) {
    $(current_cbox)
      .find(".disaster_checkbox_checkmark")
      .attr("mgh_checkbox_checked", "1");
    $(current_cbox)
      .find(".disaster_checkbox")
      .addClass("disaster_checkbox_checked_bg");
    $(current_cbox)
      .find(".disaster_checkbox_checkmark")
  ...
  if (cstate == 0) {
    $(current_cbox)
      .find(".disaster_checkbox_checkmark")
      .attr("mgh_checkbox_checked", "0");
    $(current_cbox)
      .find(".disaster_checkbox")
      .removeClass("disaster_checkbox_checked_bg");
    $(current_cbox)
      .find(".disaster_checkbox_checkmark")
...
$(document).ready(function () {
  $(".save_new_pswd").click(function (e) {
    ...
    var sidebar_width = document.querySelector(".natural_disasters")
      .offsetWidth;
...
function bind_label_col_changer_caller() {
  ...
  $(".super_node_label").click(function (e) {
    if (event.altKey) {
      ...
      $(".global_color_editor_editor_input").val(
        rgb2hex_v2($(this).css("background-color"))
      );
      $("#global_color_label_changer").removeClass("class_hidden");

      $("#global_color_label_changer").css({
        left: e.pageX,
        top: e.pageY,
      });
    }
...
$(document).ready(function () {
  $(".global_color_editor_btn_apply").click(function () {
    ...
    $(current_edited_node_col_label).css(
      "background",
      $(".global_color_editor_editor_input").val()
    );
